...
namespace tbprobe::syzygy
{
    ...
    std::string normalize_tablename(const std::string &name, bool mirror)
    {
        ...
        bool condition = (std::make_pair(w.size(), b_indices) < std::make_pair(b.size(), w_indices));
        if (mirror ^ condition)
        {
            return b + "v" + w;
        }
    ...
    bool is_tablename(const std::string &name, bool one_king, std::optional<int> piece_count, bool normalized)
    {
        return (
            (!piece_count || name.size() <= piece_count.value_or(TBPIECES) + 1) &&
            (std::regex_match(name, TABLEBASE_REGEX) &&
             (!normalized || normalize_tablename(name) == name)) &&
            (!one_king || (name != "KvK" && name[0] == 'K' && name.find("vK") != std::string::npos)));
    }

    std::vector<std::string> _dependencies(const std::string &target, bool one_king)
    {
    ...
    std::vector<std::string> dependencies(const std::string &target, bool one_king)
    {
    ...
    std::vector<std::string> all_dependencies(const std::vector<std::string> &targets, bool one_king)
    {
        std::set<std::string> closed;
        if (one_king)
        {
    ...
    std::vector<std::string> tablenames(bool one_king, int piece_count)
    {
        ...
        {
            std::string s = first + std::string(white_pieces, 'P') + "v" + first + std::string(black_pieces, 'P');
            targets.push_back(s);
            white_pieces -= 1;
            black_pieces += 1;
        }
    ...
    std::string calc_key(const Position &board, bool mirror)
    {
        ...
        return std::string(chess::popcount(board.pieces<chess::KING>(w)), 'K') +
               ...
               std::string(chess::popcount(board.pieces<chess::PAWN>(w)), 'P') +
               "v" +
     ...
     */
    std::string recalc_key(const std::vector<chess::PieceType> &pieces, bool mirror)
    {
    ...
    Table::Table(std::string path) : path(path)
    {
        ...
        std::string tablename = (last_dot == std::string::npos) ? filename : filename.substr(0, last_dot);
        ...
        else
        {
            ...
            {
                int b_count = std::count(black_part.begin(), black_part.end(), piece_type);
                int w_count = std::count(white_part.begin(), white_part.end(), piece_type);
                if (b_count == 1)
                    j++;
    ...
    void Table::init_mmap()
    {
        if (this->data == nullptr)
        {
            ...
            {
                throw std::runtime_error("invalid file size: ensure " + this->path + " is a valid syzygy tablebase file");
            }
    ...
    void Table::check_magic(const std::vector<uint8_t> &magic, const std::vector<uint8_t> &pawnless_magic)
    {
        if (!this->data)
        ...
        {
            throw std::runtime_error("invalid magic header: ensure " + this->path + " is a valid syzygy tablebase file");
        }
    }

    PairsData Table::setup_pairs(uint32_t data_ptr, uint32_t tb_size, int size_idx, int wdl)
    {
        if (!this->data)
            throw std::runtime_error("Data not initialized");
        ...
        for (int i = h - 2; i >= 0; --i)
        {
            d.base[i] = (d.base[i + 1] + this->read_uint16(d.offset + i * 2) - this->read_uint16(d.offset + i * 2 + 2)) / 2;
        }
        for (int i = 0; i < h; ++i)
        {
    ...
    void Table::set_norm_piece(std::vector<int> &norm, const std::vector<PieceType> &pieces)
    {
        if (this->enc_type == 0)
        {
            norm[0] = 3;
    ...
    uint64_t Table::calc_factors_piece(std::vector<uint64_t> &factor, int order, const std::vector<int> &norm)
    {
        std::vector<int> PIVFAC;
        if (false)
        { // this->variant.connected_kings) {
    ...
    uint64_t Table::calc_factors_pawn(std::vector<uint64_t> &factor, int order, int order2, const std::vector<int> &norm, uint64_t f)
    {
        int i = norm[0];
        if (order2 < 0x0f)
        {
    ...
    void Table::set_norm_pawn(std::vector<int> &norm, const std::vector<int> &pieces)
    {
        norm[0] = this->pawns[0];
        if (this->pawns[1])
        {
    ...
    void Table::calc_symlen(PairsData &d, int s, std::vector<int> &tmp)
    {
        ...
        if (s < 0 || s >= (int)tmp.size())
            throw std::runtime_error("calc_symlen index out of range: " + std::to_string(s));

        if (tmp[s]) return; // optional cycle guard
    ...
    uint64_t Table::encode_piece(std::vector<int> &norm, std::vector<Square> pos, std::vector<uint64_t> &factor)
    {
        int n = this->num;
        ...
        { // 111
            ...
            if (offdiag(pos[0]))
            {
                idx = (uint64_t)TRIANGLE[pos[0]] * 63 * 62 + (pos[1] - b10) * 62 + (pos[2] - j_val);
            }
            else if (offdiag(pos[1]))
            {
                idx = 6ULL * 63 * 62 + (uint64_t)DIAG[pos[0]] * 28 * 62 + (uint64_t)LOWER[pos[1]] * 62 + pos[2] - j_val;
            }
            else if (offdiag(pos[2]))
            {
                idx = 6ULL * 63 * 62 + 4ULL * 28 * 62 + (uint64_t)DIAG[pos[0]] * 7 * 28 + (uint64_t)(DIAG[pos[1]] - b10) * 28 + LOWER[pos[2]];
            }
            else
            {
                idx = 6ULL * 63 * 62 + 4ULL * 28 * 62 + 4ULL * 7 * 28 + ((uint64_t)DIAG[pos[0]] * 7 * 6) + (uint64_t)(DIAG[pos[1]] - b10) * 6 + (pos[2] - j_val);
            }
            i = 3;
        }
        else if (this->enc_type == 2)
        { // K2
            ...
            else
            {
                ...
                {
                    idx = 6ULL * 63 + 4ULL * 28 + (uint64_t)DIAG[pos[0]] * 7 + (DIAG[pos[1]] - b10);
                }
            }
            i = 2;
        }
        else if (this->enc_type == 3)
        { // 2, e.g. KKvK
    ...
    uint64_t Table::encode_pawn(std::vector<int> &norm, std::vector<Square> pos, std::vector<uint64_t> &factor)
    {
        int n = this->num;
    ...
    int Table::decompress_pairs(PairsData &d, uint64_t idx)
    {
        ...
        int64_t litidx = (idx & ((1ULL << d.idxbits) - 1)) - (1ULL << (d.idxbits - 1));
    ...
    void WdlTable::init_table_wdl()
    {
        ...
        else
        {
            ...
            for (int f = 0; f < num_files; ++f)
            {
                ...
                if (split)
                {
                    files[f].precomp[1] = setup_pairs(data_ptr, tb_size[2 * f + 1], 6 * f + 3, true);
                    data_ptr = _next;
                }
            }
    ...
    void WdlTable::setup_pieces_pawn(size_t p_data, int p_tb_size, int f)
    {
        ...
        tb_size[p_tb_size] = calc_factors_pawn(files[f].factor[0], order, order2, files[f].norm[0], f);
        ...
        tb_size[p_tb_size + 1] = calc_factors_pawn(files[f].factor[1], order, order2, files[f].norm[1], f);
    }
    ...
    int WdlTable::_probe_wdl_table(const chess::Board &board)
    {
        ...
        if (!has_pawns)
        {
            ...
            while (i < num)
            {
                ...
                chess::Bitboard bb = board.pieces(piece_type, (color == 0) ? chess::WHITE : chess::BLACK);
    ...
    std::pair<int, int> DtzTable::probe_dtz_table(const chess::Board &board, int wdl)
    {
        {
            std::lock_guard<std::recursive_mutex> lock(write_lock);
            read_count++;
    ...
    std::pair<int, int> DtzTable::_probe_dtz_table(const chess::Board &board, int wdl)
    {
        init_table_dtz();
        assert(data);
    ...
    void DtzTable::setup_pieces_pawn_dtz(size_t p_data, int p_tb_size, int f)
    {
        ...
        tb_size[p_tb_size] = calc_factors_pawn(files[f].factor, order, order2, files[f].norm, f);
    }
    /**
     ...
     */
    int Tablebase::add_directory(const std::string &directory, bool load_wdl, bool load_dtz)
    {
    ...
    int Tablebase::add_file(const std::string &path, bool load_wdl, bool load_dtz)
    {
        ...
        if (is_tablename(tablename, /*this->variant.one_king*/ true) && std::filesystem::is_regular_file(p))
        {
            if (load_wdl)
            {
                ...
                else if (tablename.find('P') == std::string::npos && ext == /*this->variant.pawnless_tbw_suffix*/ ".rtbw")
                {
                    return _open_table<WdlTable>(this->wdl, path);
                }
            }
            if (load_dtz)
            {
                ...
                else if (tablename.find('P') == std::string::npos && ext == /*this->variant.pawnless_tbz_suffix*/ ".rtbz")
                {
                    return _open_table<DtzTable>(this->dtz, path);
                }
            }
    ...
    int Tablebase::probe_wdl_table(chess::Board &board)
    {
        ...
        if (this->wdl.find(key) == this->wdl.end())
        {
            ...
            {
                throw std::runtime_error("syzygy tables support up to " + std::to_string(TBPIECES) +
                                         " pieces, not " + std::to_string(popcount(board.occ())) + ": " + board.fen());
            }
    ...
    std::pair<int, int> Tablebase::probe_ab(chess::Board &board, int alpha, int beta, bool threats)
    {
        // Check preconditions.
        /*if (board.uci_variant != this->variant.uci_variant) {
            throw std::runtime_error("tablebase has been opened for " + this->variant.uci_variant + ", probed with: " + board.uci_variant);
        }*/
        if (board.castlingRights() != 0)
        {
            throw std::runtime_error("syzygy tables do not contain positions with castling rights: " + board.fen());
        }
    ...
    std::pair<int, int> Tablebase::sprobe_ab(chess::Board &board, int alpha, int beta, bool threats)
    {
        ...
        {
            Movelist legals;
            board.legals<chess::MoveGenType::ALL &static_cast<MoveGenType>(~(int)chess::MoveGenType::PAWN)>(legals);
    ...
    std::pair<int, int> Tablebase::sprobe_capts(chess::Board &board, int alpha, int beta)
    {
    ...
    std::optional<int> Tablebase::get_wdl(chess::Board &board, std::optional<int> default_val)
    {
        try
        {
    ...
    std::pair<int, int> Tablebase::probe_dtz_table(chess::Board &board, int wdl_val)
    {
        std::string key = calc_key(board);
    ...
    int Tablebase::probe_dtz_no_ep(chess::Board &board)
    {
        ...
        if (wdl_val > 0)
        {
            int best = 0xffff;
            Movelist moves;
            board.legals<MoveGenType::ALL &static_cast<MoveGenType>(~(int)(chess::MoveGenType::PAWN | chess::MoveGenType::CAPTURE))>(moves);
            for (const auto &move : moves)
            {
                board.doMove(move);
                try
    ...
    int Tablebase::probe_dtz(chess::Board &board)
    {
        ...
        C++: raises std::runtime_error if the position could not be found in the tablebase.
         */
    ...
    std::optional<int> Tablebase::get_dtz(chess::Board &board, std::optional<int> default_val)
    {
        try
        {
    ...
    std::unique_ptr<Tablebase> open_tablebase(const std::string &directory, bool load_wdl, bool load_dtz,
