...
namespace tbprobe::gaviota
{
    ...
    constexpr int map88(int x)
    {
        return x + (x & 56);
    }

    constexpr int in_queenside(int x)
    {
        return (x & (1 << 2)) == 0;
    }

    constexpr int flip_we(int x)
    {
        return x ^ 7;
    }

    constexpr int flip_ns(int x)
    {
        return x ^ 56;
    }

    constexpr int flip_nw_se(int x)
    {
        return ((x & 7) << 3) | (x >> 3);
    }
    constexpr int idx_is_empty(int x)
    {
        return x == -1;
    }
    ...
    std::pair<chess::Square, chess::Square> norm_kkindex(chess::Square x, chess::Square y);
    extern std::array<std::array<int, 64>, 64> KKIDX;
    extern std::array<int, MAX_KKINDEX> WKSQ, BKSQ;
    std::pair<std::vector<int>, std::vector<int>> sortlists(std::vector<int> ws, std::vector<int> wp);
    struct Request
    {
        std::string egkey;
        ...
        Request(const std::vector<chess::Square> &_white_squares,
            ...
            : white_squares(_white_squares), black_squares(_black_squares),
              ...
              white_piece_types(white_types), black_piece_types(black_types), side(_side), realside(_side)
        {
            this->sortlists(white_squares, white_types);
            this->sortlists(black_squares, black_types);
        }
        void sortlists(std::vector<chess::Square> &ws, std::vector<chess::PieceType> &wp);
    };
...
std::vector<int> egtb_block_unpack(int side, int n,
    ...
    private:
        struct TupleHash
        {
            ...
            std::size_t operator()(
                const std::tuple<std::string, int, int> &t) const
            {
                std::size_t seed = 0;
    ...
    public:
        ...
        std::unordered_map<std::tuple<std::string, int, int>, TableBlock, TupleHash> block_cache;
        int block_age;
        void close();
        ...
        std::optional<int> get_dtm(chess::Board &board, std::optional<int> default_val = std::nullopt);
        ...
        ZipInfo &egtb_loadindexes(std::string egkey, std::unique_ptr<std::ifstream> &stream);
        int egtb_block_getsize_zipped(std::string egkey, int block);
        int egtb_block_park(std::string egkey, int block, std::unique_ptr<std::ifstream> &stream);
    };
