...
std::vector<uint8_t> decompress(const std::vector<uint8_t>& compressed_data, size_t uncompressed_size) {
    ...
    if ((ret != LZMA_OK && ret != LZMA_STREAM_END) || strm.avail_out > 0) {
        throw std::runtime_error("Decompression failed (Code: " + std::to_string(ret) + " - " + lzma_ret_name(ret) + ")");
    }
...
int PythonTablebase::_tb_probe(Request &req) {
    ...
    if (block_cache.find(cache_key) == block_cache.end()) {
        ...
        if (buffer_zipped[0] == 0) {
            ...
            lzma_input.insert(lzma_input.end(), buffer_zipped.begin() + 15, buffer_zipped.end());
        }
	...
	std::vector<uint8_t> buffer_packed=decompress(lzma_input, n);
        ...
        if (block_cache.size() > 128) {
            auto lru = std::min_element(block_cache.begin(), block_cache.end(),
                [](const auto &a, const auto &b) { return a.second.age < b.second.age; });
